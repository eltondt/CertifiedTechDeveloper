...
SELECT 
solicita_hospedagem.ID_Solicita_Hospedagem AS 'Identificador da Hospedágem',
solicita_hospedagem.Data_Checkin AS ' Checkin',
solicita_hospedagem.Data_Checkout AS 'Checkout',
solicita_hospedagem.Total_diarias AS 'Total de diárias',
animais.Nome_Animal AS 'Nome do PET',
clientes.Nome_Completo AS 'Nome do cliente'
FROM solicita_hospedagem
INNER JOIN hospedeiros ON hospedeiros.ID_Hospedeiro = solicita_hospedagem.ID_Hospedeiro
INNER JOIN animais ON animais.ID_Animal = solicita_hospedagem.ID_Animal
INNER JOIN clientes ON clientes.ID_Clientes = solicita_hospedagem.ID_Clientes);

SELECT * FROM vw_hospedagem;

-- CRIANDO A PROCEDURE -> INFORMO O NÚMERO IDENTIFICAR DA HOSPEDAGEM E RETORNA O NOME DO ANIMAL E O NOME DO CLIENTE


DELIMITER $$

CREATE PROCEDURE sp_hosp_anim_cli(
	IN ID_Solicita_Hospedagem BIGINT)
    
    
    BEGIN
    SELECT solicita_hospedagem.ID_Solicita_Hospedagem, animais.Nome_Animal, clientes.Nome_Completo FROM solicita_hospedagem
	INNER JOIN hospedeiros ON hospedeiros.ID_Hospedeiro = solicita_hospedagem.ID_Hospedeiro
	INNER JOIN animais ON animais.ID_Animal = solicita_hospedagem.ID_Animal
	INNER JOIN clientes ON clientes.ID_Clientes = solicita_hospedagem.ID_Clientes
    WHERE ID_Solicita_Hospedagem = solicita_hospedagem.ID_Solicita_Hospedagem;
            
    END $$
    

CALL sp_hosp_anim_cli(2);
